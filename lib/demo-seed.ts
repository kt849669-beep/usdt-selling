...
const users=names.map((name,i)=>({id:i?'m'+i:'u1',name,email:(i?'merchant'+i:'arjun')+'@nexa.example',available:(i?25000-i*1700:1250.5)*UNIT,locked:0,kyc:'verified' as const,blocked:false,merchant:i>0,orders:[24,1428,896,2301,654,1872,982][i],completion:99.8-i*.1,payment:'Demo UPI •••• '+(2400+i)}));
const offers=users.filter(u=>u.merchant).flatMap((u,i)=>(['sell','buy'] as const).map(side=>({id:side+'-'+u.id,owner:u.id,side,price:side==='sell'?9432+i*4:9421-i*3,available:(12580-i*1176)*UNIT,min:(i===2?5000:1000)*100,max:(200000+i*10000)*100,methods:i===3?['Bank transfer']:['UPI','Bank transfer'],minutes:i%2?10:15,active:true,terms:'Demo order only. Do not send real money. Use the simulated payment controls to test this trade.'})));
...
return {users,offers,orders,ledger:users.map(u=>({id:'opening-'+u.id,user:u.id,type:'Opening demo balance',amount:u.available,orderId:'',time:now,reference:'Seeded test funds; historical orders are illustrative'})),audit:[{id:'setup',actor:'System',action:'Local demo initialized',target:'Nexa P2P',time:now}],settings:{brand:'Nexa',announcement:'Trade with confidence. Every demo order uses simulated escrow.',trading:true,feeBps:0,orderMinutes:15},treasury:0};
}
