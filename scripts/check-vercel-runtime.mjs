...
const auth=moduleLoader(vars)('lib/local-auth.ts');
...
check(source('lib/server.ts').includes('creditEnabled:configured&&senderConfigured'),'Credit requires gateway and manual-delivery configuration');
check(source('lib/server.ts').includes('verifyTrc20Transfer')&&source('lib/manual-settlement.ts').includes("user.live.locked-=w.quantity"),'Withdrawals require matched transfer verification before deduction');
check(source('app/layout.tsx').includes('className="light"'),'Light theme is the default');
check(!source('components/app-login.tsx').includes("'/admin/login'"),'User login does not link to admin');
